...
static char *__b64enc(const char *str, size_t str_len)
{
...
static char *__b64dec(const char *encoded_str, size_t *len)
{
...
etcd_parse_set_response(struct json_object *etcd_resp, void *arg)
{
	...
	if (!header_obj) {
		if (etcd_debug)
			fprintf(stderr,
				"%s: invalid response, 'header' not found\n",
			       __func__);
		ev->error = -EBADMSG;
	} else {
...
int etcd_kv_put(struct etcd_ctx *ctx, struct etcd_kv *kv)
{
	...
	if (!kv->value_len)
		encoded_value = __b64enc("", 0);
	else
		encoded_value = __b64enc(kv->value, kv->value_len);
...
etcd_parse_kvs(struct json_object *resp, struct etcd_kv_event *ev)
{
	...
	if (ev->num_kvs == 0) {
		if (etcd_debug)
			fprintf(stderr, "%s: empty kvs array\n", __func__);
		ev->num_kvs = 0;
		ev->kvs = NULL;
		return;
	}
	...
	if (!ev->kvs) {
		fprintf(stderr, "%s: failed to allocate kvs\n", __func__);
		ev->error = -ENOMEM;
		ev->num_kvs = 0;
		return;
	}
	...
	for (i = 0; i < ev->num_kvs; i++) {
		...
		if (!key_obj)
			continue;
		kv->key = __b64dec(json_object_get_string(key_obj), &len);
...
etcd_parse_kvs_response (struct json_object *etcd_resp, void *arg)
{
	...
	header_obj = json_object_object_get(etcd_resp, "header");
	if (!header_obj) {
		fprintf(stderr, "%s: invalid response, 'header' not found\n",
			__func__);
		ev->error = -EBADMSG;
	} else {
		  ...
		  struct etcd_kv **ret_kvs)
{
	...
	if (!ret) {
		if (ev.num_kvs && !ev.kvs) {
			if (etcd_debug)
				fprintf(stderr,
					"%s: empty kvs returned (should be %d)\n",
					__func__, ev.num_kvs);
			ev.num_kvs = 0;
		}
		*ret_kvs = ev.kvs;
...
etcd_parse_delete_response (struct json_object *etcd_resp, void *arg)
{
	...
	if (!deleted_obj) {
		if (etcd_debug)
			fprintf(stderr, "%s: delete key failed, invalid key\n",
			       __func__);
		ev->error = -ENOKEY;
		return;
	}
	...
	if (!deleted) {
		if (etcd_debug)
			fprintf(stderr,
				"%s: delete key failed, key not deleted\n",
			       __func__);
					...
					struct etcd_kv_event *ev)
{
	...
	return;
parse_error:
	fprintf(stderr, "%s: invalid response, '%s' not found\n",
		__func__, key);
	ev->error = -EBADMSG;
}
...
int etcd_kv_txn_update(struct etcd_ctx *ctx, const char *key,
		       ...
		       char *cur_value, size_t cur_value_len)
{
	struct etcd_conn_ctx *conn;
	struct etcd_kv_event ev;
	struct json_object *post_obj = NULL;
	...
	succ_obj = json_object_new_array();
	succ_op_obj = json_object_new_object();
...
static void parse_watch_response(struct json_object *resp, void *arg)
{
	...
	if (!result_obj) {
		if (etcd_debug)
			fprintf(stderr,
				"%s: invalid response, 'result' not found\n",
			       __func__);
		ev->error = -EBADMSG;
		return;
	}
		  ...
		  struct etcd_kv_event *ev, int64_t watch_id)
{
	...
	if (ret < 0) {
		if (etcd_debug)
			fprintf(stderr, "%s: error %d executing watch request\n",
				__func__, ret);
	} else if (ev->error) {
		if (etcd_debug)
			fprintf(stderr, "%s: watch response error %d\n",
				__func__, ev->error);
		ret = ev->error;
	}
...
etcd_parse_lease_response(struct json_object *etcd_resp, void *arg)
{
	...
	if (!id_obj) {
		if (etcd_debug)
			fprintf(stderr,
				"%s: invalid response, 'ID' not found\n",
			       __func__);
		ev->error = -EBADMSG;
		return;
	}
	...
	if (!ttl_obj) {
		if (etcd_debug)
			fprintf(stderr,
				"%s: invalid response, 'TTL' not found\n",
			       __func__);
		ev->kvs->ttl = -1;
	} else {
...
int etcd_lease_grant(struct etcd_ctx *ctx)
{
	...
	if (!ret) {
		if (ev.error < 0) {
			fprintf(stderr, "%s: lease error %d\n",
				__func__, ret);
			ret = ev.error;
...
etcd_parse_keepalive_response(struct json_object *etcd_resp, void *arg)
{
	...
	if (!result_obj) {
		if (etcd_debug)
			fprintf(stderr,
				"%s: keepalive failed, 'result' not found\n",
			       __func__);
		ev->error = -EBADMSG;
		return;
	}
	...
	if (!id_obj) {
		if (etcd_debug)
			fprintf(stderr,
				"%s: keepalive failed, 'ID' not found\n",
			       __func__);
		ev->error = -EBADMSG;
		return;
	}
	...
	if (lease != ev->kvs->lease) {
		if (etcd_debug)
			fprintf(stderr,
				"%s: keepalive failed, lease mismatch\n",
			       __func__);
		ev->error = -EKEYREJECTED;
		return;
	}
...
int etcd_lease_keepalive(struct etcd_ctx *ctx)
{
	...
	if (!ret) {
		...
		fprintf(stderr, "%s: etcd_kv_exec error %d\n", __func__, ret);
...
etcd_parse_revoke_response(struct json_object *etcd_resp, void *arg)
{
	...
	if (error_obj) {
		...
		if (etcd_debug)
			fprintf(stderr, "%s: revoke error '%s'\n",
			       __func__, err_str);
		ev->error = -EINVAL;
	} else {
		ev->error = 0;
...
etcd_parse_member_response (struct json_object *etcd_resp, void *arg)
{
	...
	if (!mbs_obj) {
		if (etcd_debug)
			fprintf(stderr,
				"%s: invalid response, 'members' not found\n",
			       __func__);
		free(default_url);
		return;
	}
	...
	for (i = 0; i < json_object_array_length(mbs_obj); i++) {
		...
		for (j = 0; j < json_object_array_length(urls_obj); j++) {
			...
			if (!ctx->node_name &&
				...
				if (etcd_debug)
					printf("%s: %s skipping url %s (default %s)\n",
